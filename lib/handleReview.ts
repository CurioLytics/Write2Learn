...
export async function handleReview(flashcard_id: string, ratingValue: string) {
  ...
  if (fetchError) {
    console.error(`Supabase fetch error for ID ${flashcard_id}:`, fetchError.message);
    throw fetchError;
  }

  if (!status) {
    // Return a custom error instead of throwing an internal server error
    const notFoundError = new Error(`Flashcard status not found for ID: ${flashcard_id}`);
    (notFoundError as any).status = 404; // attach HTTP status
    throw notFoundError;
  }
